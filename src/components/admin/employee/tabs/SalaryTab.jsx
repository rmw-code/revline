import { Stack, TextField, Typography, Button, IconButton, Table, TableHead, TableRow, TableCell, TableBody, Dialog, DialogTitle, DialogContent, Paper, InputAdornment, Select, MenuItem, FormControl, InputLabel } from "@mui/material";
...
import { getEmployeeSalary, getSalaryHistory, downloadSalaryHistory, createSalaryHistory, uploadSalaryHistory } from "../../../../services/employeeService";
...
export default function SalaryTab({ form, setForm, editing }) {
    ...
    try {
      ...
      if (mime === 'application/json' || name.toLowerCase().endsWith('.json')) {
        ...
        const entry = {
          ...
          items: Array.isArray(parsed.items) ? parsed.items : (parsed.deductions ? parsed.deductions : []),
  ...
  const downloadEntry = (entry) => {
    ...
    const base = entry.baseAmount !== undefined ? entry.baseAmount : entry.baseSalary;
  ...
  return (
    <Stack spacing={2}>
      ...
      <Typography variant="subtitle2" sx={{ mt: 2 }}>Salary Items</Typography>
      ...
      <Button startIcon={<AddIcon />} onClick={() => {
        const next = [...(form.items || []), { title: '', amount: 0, type: 'OTHER' }];
        setForm({ ...form, items: next });
      }}>
        Add Item
      </Button>
      ...
      <Paper variant="outlined" sx={{ width: '100%', overflowX: 'auto' }}>
        <Table size="small">
          ...
          <TableBody>
            {history.map((h, idx) => {
              const base = h.baseAmount !== undefined ? h.baseAmount : h.baseSalary;
              ...
              return (
                <TableRow key={idx} hover>
                  ...
                  <TableCell align="right">
                    ...
                    <IconButton size="small" onClick={() => handleDownload(h)} title="Download"><DownloadIcon fontSize="small" /></IconButton>
                  </TableCell>
                </TableRow>
              )
            })}
        ...
        <DialogContent>
          {viewing && (
            <Stack spacing={1} sx={{ mt: 1 }}>
              <Typography>Base: {viewing.baseAmount !== undefined ? viewing.baseAmount : viewing.baseSalary}</Typography>
              <Typography>Net: {viewing.netAmount}</Typography>
              <Typography>EPF: {viewing.epf !== undefined ? viewing.epf : viewing.epfContribution}</Typography>
              <Typography>EIS: {viewing.eis !== undefined ? viewing.eis : viewing.eisContribution}</Typography>
            </Stack>
          )}
        </DialogContent>
      </Dialog>
